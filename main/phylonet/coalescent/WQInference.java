...
public class WQInference extends AbstractInference<Tripartition> {
	...
	public WQInference(Options inOptions, List<Tree> trees, List<Tree> extraTrees, List<Tree> toRemoveExtraTrees, List<Tree> completedTrees, List<Tree> constraintTree) {
		super(inOptions, trees, extraTrees, toRemoveExtraTrees,completedTrees,constraintTree);
	...
	WQDataCollection newCounter(IClusterCollection clusters) {
		if (constraintTree != null && constraintTree.size()> 0){
			return new WQDataCollectionConstrained((WQClusterCollection)clusters, this);
		}
		else {
